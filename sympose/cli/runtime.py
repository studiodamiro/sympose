...
async def run_command(app, command) -> None:
    ...
    if command.name == "/help":
        transcript.mount(Static("Commands:", classes="hint"))
        for c in COMMANDS:
        ...
        transcript.mount(Static("Conversation compacted (mock) — nothing to trim yet.", classes="hint"))
        ...
        transcript.mount(
            Static("Settings aren't available in the CLI yet — this is a mock.", classes="hint")
        )
...
def apply_picker_choice(app, kind: str, value: str | None) -> None:
    ...
    if kind == "model":
        ...
        if model is not None:
            ...
            transcript.mount(Static(f"Switched model to {model.label}.", classes="hint"))
    elif kind == "persona":
        persona = next((p for p in list_personas() if p.handle == value), None)
        if persona is not None:
            app.persona = persona
            picker.update_banner(app)
            transcript.mount(Static(f"Now talking to @{persona.handle}.", classes="hint"))
    elif kind == "history":
        transcript.mount(
            Static("History browsing isn't wired up yet — this is a placeholder.", classes="hint")
        )
    transcript.scroll_end(animate=False)
