...
| **`performance.max_worker_tool_turns`** | [`config.yaml`](../../../config.yaml) | `/config set performance.max_worker_tool_turns 8` | `8` (turns) | Hard ceiling on sub-agent tool calling iterations, preventing runaway loops while allowing multi-file research. |
