 ...
 * uses rather than a guessed timeout, so this can't drift out of
...
/**
 * Both read `duration-thumb` — same "enters and leaves at the same
 ...
 * — faster than content's `duration-thumb` — deliberately, so the
