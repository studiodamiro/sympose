...
from sympose.config_schema import (
    global_settings as _global_settings,
)
from sympose.config_schema import (
    persona_settings as _persona_settings,
)
...
class SymposeCompleter:
    ...
    ROOT_COMMANDS: ClassVar[list[str]] = [
        "/history",
        "/sessions",
        "/switch",
        "/setup",
    ...
    SAVE_OPTIONS: ClassVar[list[str]] = ["both", "memory", "obsidian"]

    COMMON_MODELS: ClassVar[list[str]] = [
        "list",
        "find",
        "refresh",
        "reset",
    ...
    CONFIG_KEYS: ClassVar[list[str]] = [s.key for s in _global_settings()]
    PERSONA_KEYS: ClassVar[list[str]] = [s.key for s in _persona_settings()]
    ...
    def get_personas(self) -> list[str]:
        ...
        try:
            ...
            return [
                f"@{h}"
                for h in getattr(
                    getattr(self.engine, "pm", None), "profiles", {}
                ).keys()
            ] or []
    ...
    def get_sub_agent_targets(self) -> list[str]:
        ...
        try:
            ...
            log.debug("Failed to list skill completion targets: %s", e)
        try:
            targets.extend(mcp_registry.servers.keys())
        except Exception as e:
            log.debug("Failed to list MCP completion targets: %s", e)
        return targets
    ...
    def setup_readline(cls, engine: Any) -> Optional["SymposeCompleter"]:
        ...
        for opt in (
            ...
            try:
                ...
                log.debug("readline.parse_and_bind(%r) failed: %s", opt, e)
        ...
        try:
            ...
            log.debug("Failed to set up readline history at %s: %s", hist_path, e)
