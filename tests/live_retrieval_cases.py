...
def main(verbose: bool) -> None:
    ...
    for mode in ("keywords", "embeddings", "hybrid"):
        ...
        print(
            f"{mode:11s} pass {total}/{count} ({100 * total // count}%), held-out half {held_out[0]}/{held_out[1]} | "
            + "  ".join(f"{c} {p}/{n}" for c, (p, n) in passed.items()),
            flush=True,
        )
        if verbose:
            print("\n".join(failures))
