 ...
 *
 * An earlier version shipped this at 0 (a symmetric decay-from-full-at-t=0
 * bump jumped the camera discontinuously on the flight's very first frame)
 * then fixed that with a `sin(pi * eased)` bump — but a symmetric bump
 * returns to 0 by t=1, so the last half of every flight visibly *unwound*
 * the spin, landing at the same angle a straight flight would have. The
 * twist is now one-way (0 at t=0, growing to the full angle by t=1, around
 * a random 3D axis picked fresh per flight rather than always yawing around
 * world-up), so it persists into the final framing instead of reversing
 * right before arrival. Sign no longer distinguishes zoom-in from zoom-out
 * now that the axis itself is randomised each time — both values are just
 * magnitudes.
 */
...
/**
 * Knowledge Nebula — 3D variant. A force-directed WebGL cloud of the vault:
 * notes are nodes coloured by folder and sized by link degree, `[[wikilinks]]`
 * are the edges. Paints a transparent background so whatever sits behind it
 * shows through. Lazily mounted by `<AmbientNebula>`.
 */
...
>(
  (
    ...
    //
    // three-forcegraph does not re-evaluate `nodeColor` just
    ...
    const flyCameraTo = React.useCallback(
      (
        ...
        // begins, that abrupt onset of motion right after a still
        ...
        // A fresh random axis per flight — uniform over the sphere
        ...
        const tick = () => {
          ...
          // move curls the path into a spin. An earlier version used a
          // symmetric bump (0 at both ends, so the flight always lands at
          // exactly `cameraPos`) to fix a discontinuity bug, but that meant
          // the last half of every flight visibly *unwinds* the spin it just
          // did — same start/end angle either way. This grows the angle
