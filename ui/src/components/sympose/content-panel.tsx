...
interface ContentPanelProps extends React.ComponentProps<"div"> {
   ...
   * note/folder; everything else (the section title, the tree)
      ...
      className={cn(
        ...
        phone
          ...
          : // the `mode` token, not a one-off `ease-out` — matches
            // `<MarkdownPanel>`'s own reveal transition. The odd one out
            // was most noticeable on hide: a different curve run in reverse
            // looks asymmetric next to the other panel closing alongside it.
