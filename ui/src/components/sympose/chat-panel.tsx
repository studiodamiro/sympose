 ...
 * (mention hint, ⌘↵ affordance) is reconciled with this layout. The top-right
 * action group is likewise a mock; its metrics are tuned to sit on the same
 * line as `<MarkdownPanel>`'s toolbar (8px stage inset + 8px toolbar pad + a
 * 32px control) so the two panels' chrome reads as one row. The "new
 * conversation" button in that group toggles the transcript + composer with a
 * fade-and-slide-from-below (mockup behaviour); the action row itself stays put
 * so the control remains reachable.
 */
...
function ChatPanel({
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* stage actions — parked top-right, aligned to the editor toolbar's row */}
      <div className="flex shrink-0 items-center justify-end px-3 pt-4 pb-2">
      ...
      {/* transcript + composer — toggled together: fades and slides up from
          below on reveal, back down on hide */}
      <div
        className={cn(
          // `translate-*` sets the standalone `translate` property in Tailwind
          // v4, so the transition must name it — `transform` alone leaves the
          // slide unanimated (it snaps while the fade eases).
          "flex min-h-0 flex-1 flex-col transition-[translate,opacity] duration-300 ease-out",
          bodyOpen
