...
2. **Dual-Mode 2D Vector & 3D Spatial Canvas**: A persistent background visualizer that switches between a 2D top-down planar graph and a 3D WebGL orbit space. The in-shell `2D | 3D` toggle is live (ADR-112, amending ADR-088's "Phase A" 2D-only deferral); 2D stays the default and 3D lazy-loads its `three.js` chunk only when picked. Three bugs found verifying ADR-112 are fixed: a mount-crash and a too-tight click-vs-drag threshold (ADR-114), and a duplicate background-click dispatch unique to 3D that raced the library's own click handling (ADR-115).
...
* **The app-shell ambient background (ADR-088, "Phase A"; ADR-112 enabled 3D)** — a persistent background layer behind the main menu, content panel, editor, and chat, always mounted rather than a route you navigate to:
  * **`2D | 3D` toggle** — 2D is the default and a static import (no bundle cost); 3D is a nested lazy import, so only a viewer who actually switches to it downloads `three.js`. A `three-forcegraph` mount-order crash and an over-tight click-vs-drag threshold (dropped real, non-robotic mouse clicks in both 2D and 3D) were found and patched via `patch-package` (ADR-114); a further 3D-only bug — a redundant background-click dispatch racing the library's own, causing wrong node/selection state on click — was found and removed (ADR-115), bringing 3D's click handling in line with 2D's.
  * **Click, highlight, and camera-motion refinement pass (ADR-116–120)** — five more fixes found chasing "the animation is off" and "the void still doesn't click" reports on top of ADR-112/114/115:
    * **Highlight/dim easing actually reaching the screen (ADR-116)**: the per-node highlight animation's own "still mid-fade" signal existed but was never consumed — `fg.refresh()` (three-forcegraph's rebuild hook) now only fires while at least one node is genuinely still easing, not unconditionally forever once everything had already settled.
    * **Three independent stutter causes (ADR-117)**: continuous forced camera rotation while merely dimmed (now gated strictly behind the explicit *Auto Rotate* toggle, not implied by Focus mode); unstable inline accessor functions on `<ForceGraph3D>` forcing a full digest rebuild on every render (now stable `useCallback` references reading live state through refs); and per-node colour animation routed around the library's shared-material digest entirely — each node's mesh gets its own cloned material, written to directly, with no `fg.refresh()` call needed for colour at all.
    * **Click-then-fly sequencing (ADR-118)**: the camera flight now starts ~220ms (`CLICK_ZOOM_LEAD_MS`) after the highlight colour-fade begins, so the two read as distinct events instead of one indistinct snap; a camera-spin-during-flight was built alongside this but shipped disabled pending ADR-119.
    * **One-way, random-axis camera spin (ADR-119)**: the click-triggered spin now rotates monotonically to a genuinely different final vantage point — not the symmetric bump ADR-118 shipped, which always unwound back to the same landing a straight flight would reach — around a uniformly-random 3D axis (Rodrigues' rotation formula) rather than a flat yaw.
    * **Click-vs-drag detection, corrected mechanism (ADR-120)**: supersedes ADR-114's threshold tuning — the real defect was measuring per-event `movementX`/`movementY` (a raw device delta, not real on-screen distance) rather than the threshold number itself. Both the 2D (`force-graph`) and 3D (`three-render-objects`) patches now measure cumulative on-screen displacement since `pointerdown` (`> 6px`) instead.
...
* **Pinned / Recent groups (ADR-107, ADR-108, ADR-109)**: the vault panel's list leads with a "Pinned" group scoped to the current *root* folder — a note pinned anywhere under "Daily" shows while browsing Daily regardless of depth, never mixed with another root folder's own pins — labeled by full path only when that root folder actually has subfolders (a flat one just shows the bare filename). Under it, a genuinely vault-wide "Recent" group (by name, always), resolved against the full tree regardless of which folder is in view. Each group caption offers a batch action via the same `⋯`/right-click pattern — "Unpin all" (scoped to the current root folder) / "Clear recents" (the whole history) — and Recent can be turned off entirely from Settings → Recent notes, which also sets how many show (3/5/10, default 5). A 12px gap separates each populated group from the next.
...
* **Drag-and-drop note moves (ADR-110, ADR-111)**: a note row dragged onto a folder row — a sibling folder in the tree currently in view, a root folder's own entry in the main menu, or the folder heading above the tree itself (moves the note back up to that folder's own root, since the tree only ever renders that folder's children, never a row for the folder itself) — relocates it there, via the same `PATCH /api/vault/note` a rename already uses (a slash-qualified target moves across folders, not just within one). Every drop target highlights the same way while a note is dragged over it; a drop back onto the note's own folder is a no-op that never round-trips. Folders themselves aren't draggable yet, and dragging a file in from — or out to — the OS is a separate, not-yet-built feature.
...
* **`PATCH /api/vault/note`** *(shipped, ADR-084)*:
  * Renames a note and rewrites the `[[wikilinks]]` that pointed at it. A slash-qualified `new_path` relocates it across folders in the same call — the drag-and-drop move client (ADR-110) as well as a path-qualified rename.
