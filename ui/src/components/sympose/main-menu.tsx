...
export interface MainMenuItem {
   ...
   *  `onDropNote`: only "folder" items accept the drag. */
  type?: "folder" | "note"
}
...
interface MainMenuProps extends Omit<React.ComponentProps<"nav">, "onSelect"> {
   ...
   * the note's own path, and the folder item's `id` (its
...
function MainMenu({
  ...
  return (
    <nav
      ...
      className={cn(
        // No background of its own — the ambient nebula shows
        ...
        "*:ps-(--menu-pad)",
        // The row this sits in is `pointer-events-none` so it isn't
