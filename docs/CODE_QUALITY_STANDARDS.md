...
> Drop this file in as `docs/CODE_QUALITY_STANDARDS.md` in a new project. `CLAUDE.md` and `GEMINI.md` both point here so the two assistants follow the same process instead of drifting apart. Replace `[PROJECT NAME]` below, and trim anything that genuinely doesn't apply — but don't trim anything just because it's inconvenient this week.
