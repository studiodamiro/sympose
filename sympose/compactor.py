...
class MemoryCompactor:
    ...
    def compact_file(
        ...
        prompt = (
            ...
            "2. ELIMINATE REDUNDANCY: Merge points that state the same underlying fact even if worded "
            "differently (e.g. two independently-phrased rewordings of the same preference or identity "
            "statement) into a single crisp bullet — judge by meaning, not exact text match.\n"
            ...
            "7. DROP PROCESS NARRATION: Remove any bullet describing what the assistant itself did "
            "(tool calls, sub-agents spawned, retrieval or context-configuration steps) — only facts "
            "about the user or their project are memory; how a request was processed is not.\n\n"
            f"### ORIGINAL MEMORY:\n{content_for_llm}"
        )
