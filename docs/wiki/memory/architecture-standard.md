...
2. **Tier 1 (Local LLM Triage / $0.00)**: Sub-agent running `ollama/qwen2.5:14b` or `ollama/gemma2:9b` (or `gemini/gemini-3.6-flash`) executes `sympose/builtin_skills/vault_read/SKILL.md` to parse YAML frontmatter and extract `## Key Decisions` and `## Action Items`.
